...
@onready var visual_navio: Node3D = get_node(
	"NavioMedioDentada"
)

@onready var vela_a: Node3D = get_node(
	"NavioMedioDentada/ship-pirate-medium/sail-a"
)

@onready var vela_b: Node3D = get_node(
	"NavioMedioDentada/ship-pirate-medium/sail-b"
)
...
@onready var camera_principal: Camera3D = get_node(
	"CameraPrincipal"
)
...
func destruir_jogador() -> void:
	print("O navio do Capitão Dentada foi destruído!")
